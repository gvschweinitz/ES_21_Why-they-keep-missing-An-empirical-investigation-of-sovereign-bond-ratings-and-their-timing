select_mean_sd <- function(variables,data,dateid="Date",crossid="Country",...) {
...
#       print(c("",vtn_argu[2:length(vtn_argu)]))
    ...
    # replace normalize and standardize in argument
    argu <- sub("normalize=TRUE","normalize=FALSE",argu)
    argu <- sub("standardize=TRUE","standardize=FALSE",argu)
    if (length(grep("normalize",argu))==0){argu <- paste(argu,",normalize=FALSE",sep="")}
    if (length(grep("standardize",argu))==0){argu <- paste(argu,",standardize=FALSE",sep="")}
